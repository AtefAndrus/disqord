...
| v1.11.0 | 使用統計 | サーバ/ユーザ/モデル別統計、コスト/トークン/停止率（DBマイグレーション） |
...
- 停止ボタン（🛑ボタンで進行中のリクエストを中断）
- prefix削除（`/disqord` → `/help`, `/status`, `/model`, `/config`）
...
- `setGuildModel`が既存設定を上書きするバグを修正
- 回帰テスト追加（既存設定が維持されることを確認）

### v1.3.3 (2025-12-30)

モデル詳細表示改善

- 価格表示バグ修正（100万倍して「$/1M」形式で表示、動的精度）
