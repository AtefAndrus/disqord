...
| 項目 | 要件 | 備考 |
|------|------|------|
| 単一リクエスト障害 | 他リクエストに影響を与えない | 1ユーザーのエラーで全体停止しない |
| Discord API障害 | ログ出力後、プロセス継続 | 権限エラー等でクラッシュしない |
| 未処理例外 | グローバルハンドラーでキャッチ | unhandledRejection対応 |
| エラー応答失敗 | フォールバックでログのみ出力 | 二重障害対策 |
