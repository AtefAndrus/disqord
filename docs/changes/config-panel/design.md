...
| 入力の復旧 | 検証エラーと版の衝突のときは、送信内容を下書きとして保持し、本人にだけ見える返信に「入力を修正する」ボタンを付ける。押すと下書きを入れた modal を開き直す | modal の送信への応答では、別の modal を開けない（modal を開けるのはスラッシュコマンド、ボタン、選択メニューへの最初の応答だけである）。下書きを持たないと、長い入力を最初から打ち直させることになる |
