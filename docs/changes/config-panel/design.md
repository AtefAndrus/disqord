...
| チャンネル一覧の編集 | 一覧が 24 件以下なら、一覧ごとに複数選択の Channel Select を 1 つ置き、現在の一覧を初期値（`default_values`）にする。チェックを入れると追加、外すと削除である。選択欄の `customId` に描いた時点の設定の版を入れ、押された時点の版と違えば保存せずにパネルを描き直し、選び直すよう本人に伝える。版が同じなら、保存されている一覧と送信された値の差分だけを反映する。送信された値に無いチャンネルでも、押した本人が見られないものは外さない（削除済みのチャンネルは外す）。25 件以上の一覧は、「追加」（Channel Select で選んだものを足す）と「削除」（String Select で一覧から選んで外す）の組とページ送りにする | 追加と削除で欄を分けると、一覧ごとに選択欄が 2 つ並ぶ。押されたメッセージが運ぶ初期値は、古い表示のまま操作した人が見ていた一覧とは限らないので、差分の基準には使えない。版を比べれば、選択欄を描いた一覧と保存されている一覧が同じであることを確かめられる。クライアントは本人が見られないチャンネルを選択欄に出せないので、そのチャンネルが送信された値に無くても、外す操作の結果とはみなせない。初期値で 25 件を埋めると 1 件も足せなくなるので、1 つの選択欄は 24 件以下に限る。現在の自動応答チャンネルに件数の上限は無い |
...
| チャンネル | 自動応答チャンネル、許可チャンネル | 一覧の表示と、現在の一覧を初期値にした Channel Select（25 件以上は追加の Channel Select、削除の String Select、ページ送り） |
...
- [x] `guild_settings` に `allowed_channels`、`settings_version`、`updated_by` を追加し、書き込みのたびに版を 1 増やして変更者を記録する
- [x] `/config` を単体コマンドにし、パネルを返す
- [x] 「応答」「機能」「チャンネル」「管理」のページと、`customId` の生成と解釈
- [x] ボタンと選択メニューで、書き込みのたびに認可を判定する（管理ロールの変更は `ManageGuild` のみ）
- [x] チャンネル一覧の編集（24 件以下は 1 つの Channel Select の差分、25 件以上は追加と削除とページ送り）
- [x] `messageCreate` に許可チャンネルの判定を加える
- [x] `/status` を表示に専念させ、「設定を開く」ボタンを付ける。旧形式のボタンは案内を返す
- [x] handler の単体テスト（認可の失効、指定値への変更、チャンネル一覧の差分操作、旧ボタン）
- [x] `/help` と README のコマンド一覧を更新する
- [x] 手動確認: PC とスマホで `/config` の各ページを開き、ボタンと選択メニューで設定を変えて、表示と反映を確かめる
- [x] 手動確認: 管理ロールの持ち主（`ManageGuild` なし）でギルド設定を変えられ、管理ロールそのものは変えられないことを確かめる
...
- on/off のボタンの label は「有効にする」「無効にする」だけで、設定名を含まない。ボタンは設定名と説明の TextDisplay と同じ Section に置くが、スクリーンリーダーで同名のボタンを区別できるかは未検証である。
