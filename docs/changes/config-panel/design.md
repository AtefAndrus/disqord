...
| 入力の復旧 | 検証エラーと版の衝突のときは、送信内容を下書きとして保持し、本人にだけ見える返信に「入力を修正する」ボタンを付ける。押すと下書きを入れた modal を開き直す | modal の送信への応答では modal を開けない（ボタンや選択メニューへの応答としてしか開けない）。下書きを持たないと、長い入力を最初から打ち直させることになる |
...
- 修正: `src/bot/events/interactionCreate.ts` — 選択メニューと modal の送信を上の handler へ振り分ける（現在はボタンとスラッシュコマンドだけを扱う）。旧 `/status` ボタンを案内に置き換える
...
- 下書きは `(guildId, userId, item, scope, channelId)` をキーにメモリへ保持し、15 分で捨てる。
...
- Checkbox Group を `min_values: 0` にするときは、部品に `required: false` も付ける必要がある。付けないと API が `COMPONENT_REQUIRED_ZERO_MIN_VALUES` で modal を拒否する（ドキュメントには書かれていない）。
...
- [ ] ボタン、選択メニュー、modal の送信で、書き込みのたびに認可を判定する（管理ロールの変更は `ManageGuild` のみ）
- [ ] チャンネル一覧の追加と削除（26 件以上の一覧のページ送りを含む）
- [ ] コード実行など確認の要る設定の 2 段の操作
- [ ] modal の版の比較と保存を同じトランザクションで行い、下書きと「入力を修正する」を実装する
- [ ] `messageCreate` に許可チャンネルの判定を加える
- [ ] `/status` を表示に専念させ、「設定を開く」ボタンを付ける。旧形式のボタンは案内を返す
- [ ] handler の単体テスト（認可の失効、版の衝突、入力エラー、下書きの期限切れ、旧ボタン）
- [ ] `/help` と README のコマンド一覧を更新する
- [ ] 手動確認: PC とスマホで `/config` の各ページを開き、ボタン、選択メニュー、modal で設定を変えて、表示と反映を確かめる
