...
| チャンネル一覧の編集 | 一覧が 25 件以下なら、一覧ごとに複数選択の Channel Select を 1 つ置き、現在の一覧を初期値（`default_values`）にする。チェックを入れると追加、外すと削除である。保存するのは、押されたメッセージに入っていた初期値と送信された値の差分（足したものと外したもの）だけである。26 件以上の一覧は、「追加」（Channel Select で選んだものを足す）と「削除」（String Select で一覧から選んで外す）の組とページ送りにする | 追加と削除で欄を分けると、一覧ごとに選択欄が 2 つ並ぶ。送信された値で一覧を丸ごと置き換えると、パネルを描いた後に別の人が足したチャンネルを消すので、差分だけを反映する。選択欄が初期値にできるのは 25 件までなので、それを超える一覧には使えない。現在の自動応答チャンネルに件数の上限は無い |
...
| チャンネル | 自動応答チャンネル、許可チャンネル | 一覧の表示と、現在の一覧を初期値にした Channel Select（26 件以上は追加の Channel Select、削除の String Select、ページ送り） |
...
- [x] チャンネル一覧の編集（25 件以下は 1 つの Channel Select の差分、26 件以上は追加と削除とページ送り）
