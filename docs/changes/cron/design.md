...
- 先行: [権限管理](../permissions/design.md) — cron の登録・承認・手動実行・再開は同 change の共通認可関数（`canManageGuildSettings`）で判定する。会話中の `create_cron_job` tool には、`messageCreate` がこの関数で求めた真偽値を tool コンテキストで渡す（下記「依頼者の認可を tool に渡す経路」）
...
| 権限 | 登録・承認・`run`・`resume` は [権限管理](../permissions/design.md) の `canManageGuildSettings` を満たすメンバーだけ。`pause`・`remove` は登録者本人か同関数を満たすメンバー。`list` は同関数を満たせば guild 全体、満たさなければ自分のジョブだけ | 登録と実行は Bot 運営者のクレジットを定期的に消費するので、ギルド設定の変更と同じ認可にそろえる。止める操作は消費を増やさないので、認可を失った登録者にも自分のジョブを止める手段を残す |
...
- `src/bot/events/messageCreate.ts` — [権限管理](../permissions/design.md) の `canManageGuildSettings` を `message.member` で評価して `ChatRequestContext` に載せる。応答が完了したら updater が保持した承認カードを別メッセージで送る
...
そこで `messageCreate` が応答を始める前に、[権限管理](../permissions/design.md) の `canManageGuildSettings` を `message.member` の権限とロールで評価し、その真偽値を `ChatRequestContext.canManageGuildSettings` として `chatService.generateChatResponse` に渡す。
