...
- 配信は `allowedMentions: { parse: [] }` 固定（保存プロンプト/応答に `@everyone` 等が混入しても発火させない）。`Cronjob: <name>` ヘッダを含めても合計 2000 字制限を超える場合は分割 or 末尾切り詰めが要る（分割は `src/utils/message.ts` の `splitIntoChunks` を使う）。**分割は最大 `CRON_MAX_DELIVER_CHUNKS` メッセージまで**とし、超過分は末尾切り詰め（総配信 wall-clock を有界化し `CRON_STALE_RUNNING_MS` 算定の前提を守る）。
