...
現状の DisQord は「呼ばれたら答える」受動的な bot に留まり、定期的に何かを能動的に届ける手段がない。
「平日朝 9 時に英単語を 1 つ例文付きで投稿」「毎週金曜 17 時に週報の書き方を 3 行で投稿」といった**定期 LLM タスク**を、自然言語に近い形で登録し、誰も呼びかけていなくても動かしたい。
ジョブの中では Web 検索などの tool を使わない（Non-Goals）ため、題材は外部の最新情報を要らないものに限られる。

DisQord は既に OpenRouter 呼び出し経路・スラッシュコマンド・SQLite・常駐プロセス（health HTTP サーバ）を持つため、**ジョブテーブル + ティッカー**を足すだけでチャットの仕組みを再利用して実現できる。
これは hermes-agent の cron 機能から着想を得つつ、登録時の**承認ゲート**と**自己完結プロンプトの LLM 整形**を加えて安全性を高めたものである。

## 依存 / 関連 change

- 先行: [ギルド設定変更の共通認可](https://github.com/AtefAndrus/disqord/blob/72517eb35f9d3e8928a954f83e12da2f445d9424/docs/changes/permissions/design.md) — cron の登録・承認・手動実行・再開は同 change の共通認可関数（`canManageGuildSettings`）で判定する。会話中の `create_cron_job` tool には、`messageCreate` がこの関数で求めた真偽値を tool コンテキストで渡す（下記「依頼者の認可を tool に渡す経路」）
- 連携: [使用統計](../usage-stats/design.md) — ジョブの実行ごとに `usage_logs` へ `source='cron'`、`user_id` = 登録者で記録する
...
- 前提（実装済み）: client tool の登録と実行の仕組み（`src/llm/tools/registry.ts` の `ToolRegistry` と `IClientTool`）と、Components V2 のボタン付きメッセージ。`create_cron_job` はこの上に載せる。`/cron` スラッシュコマンド経路は tool の仕組みに依存しない
...
- ジョブ実行は**会話文脈ゼロの新規 `messages` 配列**（Discord の会話履歴を読まない）で行うため、保存プロンプトは自己完結している必要がある（LLM 整形でこれを担保）
...
- ジョブからの tool 実行（コード実行・Web 検索を cron 実行で使うかは将来。v1 はプレーンなチャット応答のみ）。tool を使うと 1 回の実行コストが承認時に見積もれなくなり、無人の定期実行で課金が膨らみうるため
...
| 判断事項 | 選択 | 理由 |
| -------- | ---- | ---- |
| スケジューリングライブラリ | **croner v10**（callback なしの compute-only モード）。**`cron` の `nextRun(fromDate)` 計算にのみ使う**。`once` は croner の one-shot API（`getOnce()`）を使わず、登録時の厳格 ISO 検証 + UTC instant 保存 + 実行時 `new Date(expr)` 再パースで扱う | MIT・依存ゼロである（npm レジストリで 2026-09-24 に確認）。cron 式を IANA タイムゾーン + DST 込みで扱えるのは croner の README の記載で、Bun 上での動作とあわせて未検証のため、実装時にテストで確かめる。`new Cron(expr)` を callback なしで作るとタイマーを arm せず純粋な計算オブジェクトになる。`once` を croner に載せないのは、絶対 instant 保存で実行時計算を tz 非依存にでき、検証主体を一本化（独自 ISO 検証）できるため |
...
| 取りこぼし時 | **fast-forward は (a) 起動時リカバリ後 と (b) stale `running` 復旧後 のみ**で `computeNextRun(now)` を使い未来の最初のスロットへ前進（過去スロットを順に発火しない）。通常ティッカーは claim 時に fast-forward せず「`next_run_at <= now` を 1 回実行 → finish で完了時刻基準の次スロット計算」 | ダウンタイム/固着明けの連投を防ぐ。croner の `nextRun(fromDate)` は fromDate より**後（strictly after）**の最初の発火を返す前提で設計する（未検証。境界一致時は次スロットになるかをテストで `0 9 * * *` に `from=09:00:00` 等の境界ケースを検証）。**claim 時に fast-forward すると通常 due ジョブを 1 回も実行せずスキップする**ため、復旧経路に限定する |
...
| タイムゾーン決定 | **決定的な precedence**: 明示 `tz` オプション > `CRON_DEFAULT_TZ` > `UTC`。guild ごとの既定 tz は v1 では持たない（[settings-hierarchy](../settings-hierarchy/design.md) の対象はモデル、LLM パラメータ、システムプロンプトだけで、tz の設定は無い）。解決した tz は**承認カードに必ず表示**し、LLM に暗黙推測させない | コマンド経路と tool 経路で挙動を一致させ、DST 依存スケジュールが意図せず UTC 保存されるのを防ぐ。承認画面で tz をユーザが確認・却下できる |
...
| LLM 呼び出し経路 | `chatService` にジョブ実行専用メソッド `generateScheduledResponse(job, signal)` を足し、ジョブの `model`・settings-hierarchy 由来の system prompt・現在日時の system message を組み立てて非 stream の `OpenRouterClient.chat()` を呼ぶ。`chat()` には `signal?: AbortSignal` を足す | 既存の非 stream 経路 `chatService.generateResponse(guildId, input)` は `settings.defaultModel` 固定で、system message を 1 つも付けない（`src/services/chatService.ts:199-210`）。`OpenRouterClient.chat()` も signal を受けない（`src/llm/openrouter.ts:637`。内部の `postResponses` は受ける、同 680 行）。チャット経路が毎回付ける現在日時の system message（`buildDateTimeSystemMessage`、同 `chatService.ts:483`）が無いと、「今日の」を含むプロンプトでモデルが日付を取り違える。streaming は不要（Discord へは一括 send）。`cronService` から `OpenRouterClient` を直接呼ぶ案は、system message の組み立てと使用量の記録がチャット経路と別実装になるため採らない |
...
| 権限 | 登録・承認・`run`・`resume` は [ギルド設定変更の共通認可](https://github.com/AtefAndrus/disqord/blob/72517eb35f9d3e8928a954f83e12da2f445d9424/docs/changes/permissions/design.md) の `canManageGuildSettings` を満たすメンバーだけ。`pause`・`remove` は登録者本人か同関数を満たすメンバー。`list` は同関数を満たせば guild 全体、満たさなければ自分のジョブだけ | 登録と実行は Bot 運営者のクレジットを定期的に消費するので、ギルド設定の変更と同じ認可にそろえる。止める操作は消費を増やさないので、認可を失った登録者にも自分のジョブを止める手段を残す |
| tool 経路の承認カードの表示 | tool の `render` に承認カードの payload を載せ、`DiscordStreamingUpdater.endToolBlock` がそれを保持し、応答の完了後に `messageCreate` が返答ページとは別のメッセージとして送る（下記「tool 経路の承認カードの表示」） | 現状の `endToolBlock(_name, _render)` は `render` を捨てる（`src/bot/events/streamingUpdater.ts:399`）。返答ページはストリーミング中に分割・編集されるので、ボタン付きのカードをページ内に置くと分割と停止ボタンの行と干渉する |
| 依頼者の認可を tool に渡す経路 | `messageCreate` が `canManageGuildSettings` を評価し、結果を `ChatRequestContext` 経由で `IToolContext.canManageGuildSettings` に載せる | `isEnabled(ctx)` は同期関数で、`IToolContext` は依頼者について `userId` しか持たず、権限もロールも持たない（`src/llm/tools/registry.ts:22-29`）。メンバーの権限とロールは `message.member` に既にあるので、そこで 1 回だけ評価する |
...
- `src/llm/tools/createCronJob.ts` — `create_cron_job` client tool（既存の `ToolRegistry` に登録、承認カードを出すだけで即登録しない）
...
- `tests/unit/bot/commands/cron.test.ts`
...
- `src/services/chatService.ts` — ジョブ実行用の非 stream メソッド `generateScheduledResponse(job, signal)` を追加（ジョブの `model`、settings-hierarchy の system prompt、`buildDateTimeSystemMessage` による現在日時、`deliver_silent` 時の `[SILENT]` 規約を組み立てる。Decisions「LLM 呼び出し経路」）。`ChatRequestContext` に `canManageGuildSettings` を足し、tool コンテキストへ渡す
- `src/llm/openrouter.ts` — **`chat()` に `signal?: AbortSignal` を追加**して内部の `postResponses` へ渡す（cron の `CRON_JOB_TIMEOUT_MS` abort を非 stream 経路で効かせるために必要）
- `src/llm/tools/registry.ts` — `IToolContext` に `canManageGuildSettings?: boolean` を追加
- `src/bot/events/messageCreate.ts` — [ギルド設定変更の共通認可](https://github.com/AtefAndrus/disqord/blob/72517eb35f9d3e8928a954f83e12da2f445d9424/docs/changes/permissions/design.md) の `canManageGuildSettings` を `message.member` で評価して `ChatRequestContext` に載せる。応答が完了したら updater が保持した承認カードを別メッセージで送る
- `src/bot/events/streamingUpdater.ts` — `endToolBlock` で承認カードの `render` を保持し、`messageCreate` が読み出せるようにする
...
- **DM 非対応**: 既存 `client.ts` は `DirectMessages` intent を持たず、`messageCreate` は DM を早期 return する。よって v1 の登録経路（`/cron`・mention）も配信も guild チャンネル前提で、`guild_id` は NOT NULL とする。DM 対応は将来 change。
- スキーマ追加は既存の `applyMigrations`（`CREATE TABLE IF NOT EXISTS` + `PRAGMA table_info` で列追加判定）パターンに合わせる。時刻列は `guild_settings` の TEXT `datetime('now')` ではなく、`reply_records` と同じ epoch ms (INTEGER) を採る（分未満精度の `next_run_at` 計算と比較を一貫した数値で行うため）。
- `created_at` は INTEGER NOT NULL で SQL デフォルトを持たない（epoch ms を SQLite で素直に出せないため）。**全 INSERT は `CronJobsRepository.create()` 経由に集約**し、そこで `Date.now()` を供給する契約とする（コマンド/tool 経路が raw SQL で直接 INSERT しないことをテストで担保。列 default には頼らない）。新規 `CREATE TABLE` 時に NOT NULL 列を置く分には既存行が無いので問題ない。
...
- **NL→schedule 正規化の service 境界（`normalizeCronProposal`）**: 自然言語スケジュールの cron/ISO/interval への変換は専用サービス `normalizeCronProposal(rawSchedule, ctx, signal): Promise<{ kind, expr, tz? } | { error }>` に集約し、`/cron add` と `create_cron_job` の両経路が**同一関数**を呼ぶ（変換主体と検証主体を一本化し、両経路の挙動を一致させる）。手順:
  ...
  - (2) **真に自然言語のときだけ** OpenRouter を **1 回だけ・非 stream・tool 無効・会話文脈なし**で呼ぶ（厳格 JSON `{ kind, expr, tz? }` を出力させる固定 system prompt、モデルは現行デフォルトモデル、`CRON_JOB_TIMEOUT_MS` 級の timeout、`signal` を fetch へ伝播）。**`runToolLoop()` を呼ばず・Discord の会話履歴も読まない**（再帰的な tool ループや会話状態への依存を作らない）。system message には現在日時（`buildDateTimeSystemMessage`）を含める（「明日の朝」などの相対表現を解決するため）。
...
- ジョブ実行 `runScheduledJob(job, signal)` は `chatService.generateScheduledResponse` を呼び、Discord の会話履歴を読まずに**毎回新規の OpenRouter `messages` 配列**を組む。保存 `prompt` を user メッセージとし、guild/channel の system prompt を前置（settings-hierarchy 未成立時は前置なし）。チャット経路と同じく、現在日時の system message（`buildDateTimeSystemMessage`、Web 検索なしの文面）を毎回付ける（定期ジョブは「今日の」「今週の」を含むことが多く、日付が無いとモデルが学習時点の日付で書く）。実行後は [使用統計](../usage-stats/design.md) の `usage_logs` に `source='cron'`・`user_id` = 登録者で記録する（同 change が先に実装されている場合）。**`deliver_silent=1` のジョブは system prompt 末尾に `[SILENT]` 規約**（特筆事項が無ければ本文を `[SILENT]` のみにせよ）を注入する（フラグ 0 なら注入しない＝常に通常応答）。tool は v1 では渡さない。実 LLM 呼び出しは上記「LLM 呼び出し経路」決定に従い、**`model` 非 NULL ならそのモデル、NULL なら実行時の現行デフォルトモデル**で行い、`signal` を OpenRouter fetch へ通して timeout 時に abort する。
...
- 配信先 preflight `resolveSendable(channelId, timeoutMs): Promise<SendableChannel | null>`: `client.channels.fetch(channelId)` を `timeoutMs` で囲み（hang 防止）、**`null` / 非テキストチャンネルを返しうる**ため `ch?.isTextBased() && ch.isSendable()`（discord.js v14 の sendable 判定）で**型ガード**し、さらに bot の実効権限（`ch.permissionsFor(client.user)`）が `ViewChannel` と `SendMessages`（スレッドなら `SendMessagesInThreads`）を含むときだけ返す（fetch reject/timeout は内部 catch → `null`）。discord.js 14.27.0 の `isSendable()` は `send` メソッドの有無しか見ないので、送信権限を失ったチャンネルも通してしまう。戻り型は discord.js の sendable 系 union（`isSendable()` が絞り込む型。明示するなら `Extract<Channel, { send: (...args: never[]) => unknown }>` 相当のローカル別名 `SendableChannel`）にして、`deliver(ch, ...)` が strict TS 下で `ch.send(...)` を型付きで呼べるようにする。**LLM 呼び出しの前**に実行し、不能なら LLM を呼ばずに `onFailure` へ（削除済みチャンネル等で毎回 OpenRouter 課金が走るのを防ぐ）。preflight 通過後に**最初の `send`** が throw（一時的権限変動）して 0 チャンクなら catch → `onFailure`（2 チャンク目以降の throw の扱いは下記「分割配信の会計ポリシー」で success 側）。
- 配信は `allowedMentions: { parse: [] }` 固定（保存プロンプト/応答に `@everyone` 等が混入しても発火させない）。配信はチャットの返答と同じ Components V2 の payload（`src/utils/chatContainerBuilder.ts`）で組み、`Cronjob: <name>` のヘッダを含めて 1 メッセージの予算を超える場合は `splitTextIntoMessages` で分割する。**分割は最大 `CRON_MAX_DELIVER_CHUNKS` メッセージまで**とし、超過分は末尾切り詰め（総配信 wall-clock を有界化し `CRON_STALE_RUNNING_MS` 算定の前提を守る）。
...
- `create_cron_job` tool（LLM 起点）: LLM が `{name, schedule, prompt, channel?, once?}` を渡す。tool ハンドラは**登録せず**同じ承認カードを `render` で返し、応答の完了後に当該チャンネルへ別メッセージとして送られる（下記「tool 経路の承認カードの表示」）。Approve で初めて INSERT。
- 承認は **共有 in-memory 承認マネージャ**で扱う（コマンド経路と tool 経路で統一）: nonce をキーに「正規化済み・検証済み payload + 登録者 id + guild/channel scope + 期限」を保持し、Approve/Reject ボタンの custom_id に nonce を載せる（例 `cron:approve:<nonce>` / `cron:reject:<nonce>`）。押下は**グローバル `interactionCreate` のコンポーネントハンドラ**が custom_id prefix で振り分ける。**`awaitMessageComponent`（メッセージ別コレクタ）は使わない**: tool 経路ではカードが tool handler の戻り後、応答の完了時に送られるため、handler 内でそのメッセージを await できないため。押下者が登録者本人であり、かつ `canManageGuildSettings` を満たすことをハンドラ内で検証する。
...
- **Approve 時に権限・scope を再チェック**: 押下者が登録者本人であることを確認するだけでなく、**INSERT 直前に登録者の現在の権限と対象 scope（guild/channel への送信権・登録権）を再評価**し、カード提示〜Approve の間に権限喪失/チャンネル変更があれば INSERT しない（理由を提示）。承認待ちの時間差で失効した権限のまま登録されるのを防ぐ。
- **配信先チャンネルの guild 帰属を必須検証**（`channel?` override 対策）: ユーザ/LLM が `channel?` を渡した場合、解決したチャンネル（スレッドなら親チャンネル）の guild が**ジョブの `guild_id`（= `ctx.guildId`）と一致する**ことを INSERT 直前に検証し、**別 guild のチャンネル ID は bot が送信可能でも拒否**する（cross-guild 配信の防止）。あわせて (a) bot が**その特定チャンネル**へ送信権を持つ（`channel.permissionsFor(client.user)`）、(b) 登録者が**そのチャンネル**を閲覧でき、`canManageGuildSettings` を満たすことを確認する。`channel?` 省略時は登録元チャンネル（`ctx.channelId`）を使い、これは定義上同一 guild なので帰属チェックは自明に満たす。
...
既存の `IClientTool`（`src/llm/tools/registry.ts`）として実装し、`ToolRegistry.register()` で登録する。既存の handler 契約（`handler(args, ctx, signal, meta): Promise<{ llmResult: string; render? }>`・`validate`・`isEnabled`）に厳密に従う。**この tool は DB に INSERT しない**（承認カードを描画し、Approve 経由でのみ登録される。下記）。

- `name`: `"create_cron_job"`。`description`: 「定期/単発の LLM タスクの**登録を提案**する（自動登録はしない・承認が必要）」と明記し、モデルに即時副作用がないことを伝える。
- `parameters`（JSON Schema）: `{ name, schedule, prompt, channel?, once?, timezone?, model?, silent? }`。`schedule` は自然言語/cron/ISO/インターバルのいずれかを受ける文字列、`once` は単発フラグ（省略時は schedule から kind 推定）。`silent?` は `/cron add` の `silent?` と同義（`deliver_silent` に保存・既定 false）。`timezone?` は IANA tz の明示指定で、`/cron add` の `tz?` と**同一の precedence・検証**（明示 > `CRON_DEFAULT_TZ` > `UTC`）に乗せる。`model?` は `/cron add` の `model?` と同義（非 NULL = 登録時スナップショット固定、省略 = 動的デフォルト解決）。cron/interval は `expr` 自体が tz を持たないため、tool 経路でも明示 tz を運べないと guild/既定 tz へ暗黙フォールバックしてしまう点を防ぐ（解決 tz は承認カードに必ず表示）。
- `isEnabled(ctx)`: 次を全て満たすときだけ true（false なら基盤が tool を提示せず、dispatch 時にも再評価して error 結果にする）:
  - `CRON_ENABLED === true`。
  - `ctx.toolsAllowed !== false`（モデルが function calling に対応する）。`ToolRegistry.buildTools()` は判定を各 tool の `isEnabled` に任せるので、この条件は tool 側で書く。現在の `generateChatResponse` は会話履歴が有効なときにしか `supportsTools` を判定しないので、[Discord 操作ツール](../discord-tool/design.md) と [画像生成](../image-generation/design.md) と同じく、`CRON_ENABLED` のときにもモデルの詳細を取得して判定する。
  - **`ctx.guildId !== null`**（`cron_jobs.guild_id` は NOT NULL・DM 非対応のため、DM/guild 外コンテキストでは tool 自体を無効化する）。
  - `ctx.canManageGuildSettings === true`（`messageCreate` が依頼者について評価した値。未設定は false と同じに扱い、認可を満たさない依頼者にはモデルが登録を提案できない）。
...
`render` は tool ループから updater の `endToolBlock(name, render)` へ渡るが、現状の `DiscordStreamingUpdater.endToolBlock` は何もしない（`src/bot/events/streamingUpdater.ts:399`）。
本 change で次の経路を足す。

1. `create_cron_job` の handler は `render` に `{ kind: "cron-approval", nonce }` を返す。カードの中身（プロンプト全文、解釈したスケジュール、次回実行時刻、タイムゾーン、配信先、silent モード）は承認マネージャが nonce に紐付けて持つ。
2. `DiscordStreamingUpdater.endToolBlock` は `kind === "cron-approval"` の `render` を受け取ったら nonce を内部の配列に積むだけで、Discord には書き込まない。tool ループは updater の呼び出しを時間で打ち切ることがあるが、呼び出し自体は必ず行う（`src/llm/toolLoop.ts` の `IToolLoopUpdater` のコメント）ので、同期的に積めば取りこぼさない。
3. `messageCreate` は応答が `final` で終わり、返答ページの最終描画を終えた後に、積まれた nonce ごとに承認マネージャからカードを組み立て、依頼メッセージへの返信として別メッセージで送る。カードは Components V2 のコンテナに Approve / Reject ボタンを付けたもので、`allowedMentions: { parse: [] }` で送る。
4. 応答が停止・エラーで終わった場合はカードを送らず、積まれた nonce を承認マネージャから破棄する。依頼者が止めた応答から登録が始まることを避けるためである。

カードを返答ページの中に置かないのは、返答ページがストリーミング中に分割と編集を繰り返し、生成中は停止ボタンの行を持つためである。
ボタン付きのカードを別メッセージにすれば、ページ分割の計算にも停止ボタンの付け外しにも関わらない。

### 依頼者の認可を tool に渡す経路

`isEnabled(ctx)` は同期関数で、`IToolContext` は依頼者の権限を持たない（`src/llm/tools/registry.ts:22-29`）。
そこで `messageCreate` が応答を始める前に、[ギルド設定変更の共通認可](https://github.com/AtefAndrus/disqord/blob/72517eb35f9d3e8928a954f83e12da2f445d9424/docs/changes/permissions/design.md) の `canManageGuildSettings` を `message.member` の権限とロールで評価し、その真偽値を `ChatRequestContext.canManageGuildSettings` として `chatService.generateChatResponse` に渡す。
`chatService` は tool ループに渡す `IToolContext` に同じ値を載せる。
`message.member` が取れない場合は false とする。
承認マネージャは Approve の押下時に、押下したメンバーの権限で同じ関数をもう一度評価する（応答から押下までの間に権限が変わりうるため）。
...
- `<id>` は DB の `id`。操作の前に、対象ジョブの `guild_id` が実行元の guild と一致することと、Decisions「権限」の認可（`add`/`run`/`resume` は `canManageGuildSettings`、`pause`/`remove` は登録者本人か同関数）を確かめる。
...
各項目の仕様は Design の該当節を正とし、ここには作業の単位だけを書く。

- [ ] `croner` 依存追加（実装時点の最新安定版を確認する。参照節）
- [ ] `cron_jobs` テーブルと `CronJobsRepository`（Design「DB スキーマ」「変更対象ファイル」）
- [ ] `OpenRouterClient.chat()` に `signal` を追加し、`chatService.generateScheduledResponse` を実装（現在日時の system message を含む。Decisions「LLM 呼び出し経路」）
- [ ] `cronService`: スケジュール正規化・検証（Design「スケジュール正規化・検証」）
- [ ] `cronService`: ティッカー、claim、配信、失敗処理、stale 復旧、起動時リカバリ（Design「実行（ティッカー）」）
- [ ] `src/index.ts` の配線、起動時リカバリ、ティッカー開始、`shutdown` への追記（`CRON_ENABLED=false` では cron state を変更しない）
- [ ] 承認マネージャと `interactionCreate` の `cron:approve|reject:<nonce>` 振り分け（Design「登録の承認フロー」）
- [ ] `/cron` コマンド群と status 別の操作可否（Design「コマンド設計」）
- [ ] `IToolContext.canManageGuildSettings` と `messageCreate` での評価（Design「依頼者の認可を tool に渡す経路」）
- [ ] `create_cron_job` client tool と、承認カードを別メッセージで送る updater / `messageCreate` の経路（Design「`create_cron_job` client tool の契約」「tool 経路の承認カードの表示」）
- [ ] 使用統計が実装済みなら、ジョブ実行を `usage_logs` に `source='cron'` で記録する
- [ ] 環境変数（`CRON_*`）を `envVars.ts` に追加
- [ ] `/status` に cron 機能状態（有効・登録数）を表示
- [ ] テスト: Design の各節に書いた不変条件ごとに 1 つ以上のケースを置く。特に、claim と `claim_id` 突合による二重実行・遅延書き込みの防止、kind 別の起動時リカバリと stale 復旧、`resume` の null ガード、分割配信の会計、Approve の原子性、境界一致の `nextRun`、秒粒度と下限の拒否、tool 経路のカードが停止時に送られないこと
- [ ] README AUTO セクション再生成（コマンド一覧 + 環境変数）
- [ ] `docs/changes/cron/` 削除（リリース完了時、git 履歴がアーカイブ）

## Open Questions / Risks

- **連続失敗時の登録者通知 UX**: 自動停止ロジック（`fail_count >= CRON_MAX_FAILURES` で `paused`）は確定。残るのは**通知先**。DM の送信は REST（Create DM とメッセージ送信）で行え、`DirectMessages` intent は DM の受信にしか要らない。ただし登録者がサーバーメンバーからの DM を閉じていると送信は失敗する。登録者への DM、元チャンネルへのメンション付き通知、停止だけしてサイレントにする（`/cron list` の状態表示で気付かせる）のどれにするか、DM が失敗したときにどれへ落とすかが未確定。
...
- **タイムゾーン入力**: precedence（明示 > `CRON_DEFAULT_TZ` > `UTC`）と承認カードでの明示表示は確定（Decisions 参照）。残るのは `CRON_DEFAULT_TZ` の運用既定値（`UTC` か運用者ローカルか）と、guild ごとの既定 tz が要るかどうか（要るなら置き場所を決める別 change）。
...
- **多重起動 / stale `running` 復旧**: v1 は単一プロセス想定で、(a) 起動時に全 `running` を kind 別に復旧する起動時リカバリ + (b) 毎 tick 先頭で `claimed_at` が古い `running` を復旧する常駐 sweeper、で固着を回収する（Design「実行（ティッカー）」の stale `running` 復旧）。多重起動を許す場合は起動時に全 `running` を対象にする復旧が他プロセスの実行中ジョブを誤って戻すため、起動時も `claimed_at` ベースの time-boxed 判定に変え、かつリーダー選出が要る。v1 ではスコープ外。
...
- [croner](https://github.com/hexagon/croner) — MIT・依存ゼロ。2026-09-24 時点の npm の最新安定版は 10.0.1 で、11.0.0-dev.0（2026-08-31 公開）が dev タグにある。実装時にメジャー版が上がっていないかを確認する。Bun 上での動作と `nextRun(fromDate)` が fromDate より厳密に後を返すことは未検証で、テストで確かめる。本設計では **`cron` の `new Cron(pattern, { timezone })` + `nextRun(fromDate)` にのみ使用**。IANA tz / DST 対応・不正パターンは構築時 throw。`once` は croner の one-shot API（`new Cron(date)` + `getOnce()`）を使わず独自に厳格 ISO 検証 + UTC instant 保存（理由は Decisions「スケジューリングライブラリ」）
- [OpenRouter Responses API](https://openrouter.ai/docs/api_reference/responses/overview) — `OpenRouterClient.chat()` は内部で Responses API を非 stream で呼ぶ。ジョブ実行は新規 `messages` 配列での単発呼び出し
