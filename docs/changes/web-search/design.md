...
| 検索結果の表示 | 回答の後ろに、モデルに渡った検索結果のページを最大 5 件リンクで並べる。検索語と結果 URL はログに出す | 利用者が回答の根拠を開けるようにし、運用者が検索の中身を後から確かめられるようにする |
...
- これらが効く範囲は HTTP リクエスト 1 回である。`runToolLoop()` の `MAX_TURNS` は 5 で最終ターンは `tool_choice: "none"` を送るため、client tool を併用する応答では server tool を載せたリクエストが最大 4 回になり、検索料金の上限も 4 倍になる。
...
- 回答の後ろに `-# 検索結果` の小見出しと、検索結果のページを最大 5 件、`[タイトル](<URL>)` の形で並べる。本文に足してから分割するので、長い回答では最後のページに載る。タイトルと URL は外部のページ由来なので、http(s) 以外の URL と空白や `<>` を含む URL は捨て、タイトルからはリンク記法やメンションを作りうる文字と `://` を除く（URL に見えるタイトルは、リンク先と違う場所へのリンクに読めるため）。URL を `<>` で囲み、Discord の埋め込みプレビューを出さない。
