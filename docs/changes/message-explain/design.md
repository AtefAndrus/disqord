...
- 連携: [設定パネル（/config の再構成）](../config-panel/design.md) — 同 change が定めるチャンネル制限（`allowed_channels`）は解説にも掛け、許可チャンネルの外のメッセージに対する解説は断る（理由は Open Questions / Risks の「チャンネル制限との関係」）。
