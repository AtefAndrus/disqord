...
- 前提（実装済み）: [web-search](https://github.com/AtefAndrus/disqord/blob/5f1bfa49759e1d5ee74e97718d61adff81f2b601/docs/changes/web-search/design.md) — Web 検索の設定が有効なサーバーでは、`generateChatResponse()` が検索の server tool を付ける。解説経路も同じ関数を通るので、時事的な背景や新しい用語の解説にも同じ設定で検索が使われる。
- 関連: [ギルド設定変更の共通認可](https://github.com/AtefAndrus/disqord/blob/72517eb35f9d3e8928a954f83e12da2f445d9424/docs/changes/permissions/design.md) — 解説の実行はギルド設定の変更ではないので、同 change の共通認可関数は呼ばない。
- 連携: [設定パネル（/config の再構成）](https://github.com/AtefAndrus/disqord/blob/67291dde3d1ca40f9243d10430e98c3600341dbf/docs/changes/config-panel/design.md) — 同 change が定めるチャンネル制限（`allowed_channels`）は解説にも掛け、許可チャンネルの外のメッセージに対する解説は断る（理由は Open Questions / Risks の「チャンネル制限との関係」）。
...
- 解説は実行した本人にだけ見える（ephemeral）返信で出し、通常のチャット返信と同じくストリーミング表示、停止ボタン、長文の分割、LLM 詳細表示の設定による footer を備える。
...
| モデル | サーバーの既定モデル（`/model set`） | 無料モデル限定を含む既存の設定がそのまま効く。解説専用のモデル設定は、必要になった時点で settings-hierarchy に載せる |
...
`interaction.targetMessage` を代わりに使うことはしない。
discord.js は対象メッセージもキャッシュへ上書きで合成して返す（`node_modules/discord.js/src/structures/ContextMenuCommandInteraction.js:56`）ので、その後に同じメッセージの古い snapshot を含む転送を受け取ると、対象の内容が snapshot の内容に書き換わりうるためである。
...
- [ ] 手動確認: 開発サーバーでメッセージを右クリック → アプリ → 解説する を実行し、本人にだけ見える解説がストリーミング表示され、長文なら分割され、LLM 詳細表示の設定が有効なら footer が出ることを確かめる
...
- **チャンネル制限との関係**: 設定パネルのチャンネル制限（`allowed_channels`）が設定された guild では、許可チャンネルの外のメッセージに対する解説を ephemeral のメッセージで断る。解説は本人にだけ見えチャンネルに何も残さないが、OpenRouter の消費は通常の返信と同じく生じ、チャンネル制限は bot がクレジットを使う場所を絞るための設定だからである。本 change が先に実装される場合、この確認は設定パネルの実装時に足す。
