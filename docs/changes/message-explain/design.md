...
- **チャンネル制限との関係**: permissions のチャンネル制限（`allowed_channels`）が設定された guild では、許可チャンネルの外のメッセージに対する解説を ephemeral のメッセージで断る。解説は本人にだけ見えチャンネルに何も残さないが、OpenRouter の消費は通常の返信と同じく生じ、チャンネル制限は bot がクレジットを使う場所を絞るための設定だからである。本 change が先に実装される場合、この確認は permissions の実装時に足す。
