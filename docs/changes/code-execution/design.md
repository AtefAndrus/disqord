...
| queue timeout の結果形 | **正常 `ExecResult`**（`systemError.kind:"queue_timeout"`）として返し、throw しない。例外は **foundation `signal` abort（request cancel / per-tool wall timeout の双方）のみ**で、これは foundation の cancellation 契約に従い throw する | キュー枯渇は「実行できなかった」想定内事象であり、`args.timeout_sec`（sandbox 内 timeout）と同じく LLM へは tool result、Discord へは red Container で一貫描画したい。`/run` と LLM tool 経路の両方で同じ整形経路に乗る。`signal` abort だけは「結果不明・ターンを進めない」ため throw（cancel/timeout の分類と結果生成は foundation が signal 状態で行う） |
...
interface ISandboxService {
  ...
  //      ★ teardown は SANDBOX_TEARDOWN_DEADLINE_MS で bound。**超過時は下記「teardown 超過の単一ステートマシン」**（handler は即 settle、残り teardown は permit 保持の detached cleanup へ、最終的に有界予算切れで permit 強制解放 + leak warn）。これが foundation の killable isolation 契約を満たす
  ...
  //   - **同時数 permit はこの detached cleanup 完了まで保持する**: concurrency permit（Bottleneck slot 相当）を
  ...
  //     orphan create が積み上がり実 VM 数が N を超えうる。よって permit の解放は **(success) teardown 完了時 / (abort・
  //     boot-timeout) 上記 detached cleanup 継続の完了時**に行い、handler の早期 throw とは切り離す（permit のライフ
  //     タイムは「sandbox を物理的に始末し終えるまで」= handler return より長くなりうる）。これで active + 生成中 +
  //     teardown 中の合計が常に N 以下に bound される。
  ...
  //     ・**`sandbox.kill()`（= ユーザコード + ネットワークを物理的に停止する安全境界）は必ず完了させる**。これが foundation の
  //       killable isolation 契約の本体なので、abort 経路の throw 前にも `sandbox.kill()` の完了を待つ（best-effort で済ませない）。
  //       `sandbox.kill()` 自身を有界リトライ（短い deadline × 数回）で叩き、ハングに備える。
...
| `SANDBOX_TEARDOWN_DEADLINE_MS` | `10000` (10s) | `stopAndWait()`/`kill()`→`removePersisted()` の **handler 同期待ち**サブデッドライン。超過時は handler を即 settle し残り teardown を permit 保持の detached cleanup へ切り出す（「teardown 超過の単一ステートマシン」）。wall budget の teardown 項を bound（detached 側の有界リトライ予算は別） |
