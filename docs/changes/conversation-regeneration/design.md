...
- 先行（**確定済み・必読**）: [conversation-context](../conversation-context/design.md) — `sessions`/`turns`/`turn_messages`、`status`/`active`/`finalized_at`、`parent_user_turn_id`/`abandoned`、`idx_turns_one_assistant`（`failed` を除く部分 UNIQUE）、`WHERE status='pending'` による確定、exchange 単位の境界/予算/保持、`messageUpdate` を無視。本 change はこれらの契約の上に載る
- 先行: [会話履歴の削除同期の強化](../conversation-context-sync/design.md) — 本 design が前提にしている内部削除の lease（`deleting_internal_at`）、起動時の補償削除（pending reconcile）、文脈に使う直前の REST 再検証、reply seed は conversation-context には無く、この change が所有する。本 design のうちそれらに触れる記述（スキーマ移行の前提、打ち切った世代の始末、compaction の削除検証、reply seed の pierce、Discord 表示の削除）は、同 change の設計が決まった時点で合わせて見直す（要見直し）。lease の採否とは別に、conversation-context の次の契約とも合わせる必要がある（要見直し）: 写像の追加は `pending` の turn にだけ行えるので、確定の後に追加送信して写像を足す §2.2 の順序はそのままでは写像が作られない。確定時に `finalized_at` を書かない世代は、文脈の選択（`finalized_at` が今回の発言以下の assistant だけを入れる）から外れる。また、conversation-context は削除を raw gateway イベントで受け、`Partials.Message` を client に追加しない。編集起因の再生成で使う `messageUpdate` をキャッシュに無いメッセージでも受けるための partials の設定は、本 change が導入する
