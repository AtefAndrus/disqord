...
- 関連: [Responses API への移行](../responses-api-migration/design.md) — server tool の送り方は両 API で同じだが、実行が独立した output item として観測できるようになる。実 wire の fixture 化は移行後に行う
...
| subagent の nested tools | `tools` と `max_tool_calls`（上限 25）がある。さらに `inherit_functions` / `inherited_function_names` で client function tool を継承できる（EXPERIMENTAL） | 公開スキーマ `SubagentServerToolConfig` で確認。「function tool は原理的にネストできない」という前提で設計を固めない |
...
- (a) 各 server tool を有効化する呼び出し側が、その server tool の `parameters` で **1 起動あたりの**コストを縛る（本 change の 4 ツールに効く制御は下記。`max_results`/`max_total_results` は `web_search` 用で本 change のスコープ外）:
  ...
  - `advisor`: 回数の knob は無い。`parameters.model` と `max_completion_tokens` でコストが決まる（top-level の会話モデルとは別）。
...
- **パラメータ**（現行ドキュメントと公開スキーマ `FusionServerToolConfig` で確認）: `analysis_models`（panel、1–8。既定は `~anthropic/claude-opus-latest` / `~openai/gpt-sol-latest` / `~google/gemini-pro-latest` という品質プリセット）/ `model`（analyst。既定は外側のリクエストと同じモデル）/ `max_tool_calls`（既定 4、範囲 1–16）/ `tools` / `max_completion_tokens`（既定 16,000、推論トークンを含む）/ `reasoning` / `temperature` / `cache_control`。
  ...
  - 各 panel モデルは web_search と web_fetch を**有効にした状態で並列実行**される。`tools` に空配列を渡せば内部ツールを無効化できるとされているが、実 API では未確認。
...
- **パラメータ**（現行ドキュメントと公開スキーマ `AdvisorServerToolConfig` で確認）: `name` / `model`（相談先、既定は外側のリクエストと同じモデル）/ `instructions` / `forward_transcript`（既定 false）/ `stream`（Responses API のみ）/ `max_completion_tokens` / `reasoning` / `temperature`。
...
- **コスト**: 相談先 `model` と `max_completion_tokens`（推論トークンを含む）で決まる。上位モデルを指定すると高コストになる。
...
- **client function tool を継承できる（EXPERIMENTAL）**: `inherit_functions` を真にすると、subagent が呼び出し側の client function tool をすべて継承する。`inherited_function_names` で継承する function tool の名前を絞れる。どちらも「EXPERIMENTAL — subject to change without notice」と注記されているので依存はできないが、**「function tool は原理的にネストできない」という前提で設計を固めてはいけない**。
- **nested tools の allowlist（要設計）**: `parameters.tools` は outer request とは別の allowlist。outer の `serverTools` を素通し転送せず、既定は空 or 低コスト server tool の curated subset に絞る。再帰 subagent の可否と、`inherit_functions` を使うかどうかを明示的に決める（採用時に確定）。
...
- [ ] `subagent` の `inherit_functions` を有効にしたときの実挙動を実 API で確認（EXPERIMENTAL 表記のため仕様変更を前提に扱う）。あわせて nested `tools` の allowlist（既定の curated subset・再帰 subagent の可否）を設計
...
- **nested tools の allowlist**: `subagent` の `parameters.tools` は outer request とは別 allowlist。outer の `serverTools` を素通しすると再帰 subagent や高コスト tool まで許可しうる。既定 subset と再帰可否を採用フォルダで確定する。`advisor` にはこの論点が無い。
...
- [OpenRouter Subagent Server Tool](https://openrouter.ai/docs/guides/features/server-tools/subagent) — `openrouter:subagent`。`name`/`model`/`instructions`/`tools`/`max_tool_calls`(上限 25)/`max_completion_tokens`/`reasoning`/`temperature`/`inherit_functions`/`inherited_function_names`。返り `{status, model, task_name, outcome}`
