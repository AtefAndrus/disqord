...
- **client function tool を継承できる（EXPERIMENTAL）**: client function tool を subagent に使わせる経路は `tools` ではなく `inherit_functions` である。真にすると、subagent が外側のリクエストの `tools` にある client function tool をすべて継承する。`inherited_function_names` で継承する function tool の名前を絞れる。`inherit_functions` は Responses API（`/api/v1/responses`）でだけ受け付けられ、他の API では 400 になる。どちらも「EXPERIMENTAL — subject to change without notice」と注記されているので依存はできない。
