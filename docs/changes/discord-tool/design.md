...
- カスタム絵文字に使えるロールの制限（`roles` が空でない）があるときは、bot と依頼者の双方がそのロールのどれかを持つときだけ使う。制限は、下の名前の解決で REST から取った絵文字から読む。
...
- `add_reaction` のカスタム絵文字は、実行の直前に `guild.emojis.fetch()` で guild の絵文字の一覧を REST から取り、その中から名前で引く。キャッシュを使わないので、起動後に追加や改名された絵文字も引ける。見つからなければ実行せず、使える絵文字が無いことを返す。
