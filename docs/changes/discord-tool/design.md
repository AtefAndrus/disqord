...
| `fetch_recent_messages` の content | `channel.messages.fetch({limit})` は intent 無しでも構造（id/author/timestamp）は取れる。**本文（content/embeds/attachments）は MessageContent 特権 intent（portal トグル）に依存して空になりうる**。content 不在時は tool 結果にその旨を明記 | content 欠落を黙って空返しすると LLM が「無言だった」と誤認するため明示する。**ただし REST fetch が実際に gating されるかは Discord docs 上 contested**（canonical Message Resource は空 content ルールを **GATEWAY イベント**〔`MESSAGE_CREATE`/`MESSAGE_UPDATE`〕の文脈に限定する一方、field report〔discord-api-docs #4552/#4775〕は REST も portal トグルで gating されると報告）→ **実装時に実 API で確認**。設計はどちらでも頑健（flag で tool を hard-gate せず、注記は**実際に content が空のときだけ**付ける） |
...
buildTools(ctx):
  available = intentAvailableActions(ctx.appFlags)   // ApplicationFlags 由来 upper bound（v1 は全 action）
              ∩ ctx.allowlist                        // カテゴリ別設定（read on / member-lookup・management off）。ctx に事前解決済み
  → isEnabled(ctx)===true の tool だけ tools 配列へ
  // isEnabled は同期。allowlist / context 有効性（reader 有無）/ per-caller ゲート設定 / channel /
  // fresh invoking member / appFlags はすべて ctx 構築時（per-response）に事前解決済みの値だけを読む（DB/REST fetch しない）。
...
- `src/llm/tools/registry.ts`（基盤）への登録呼び出しを `index.ts`（DI 構成）から行う。`IToolContext` に Discord 依存（client/message 参照、`ctx.channel`〔種別判定用〕、appFlags キャッシュ、history reader〔context 有効性は reader 有無で同期判定〕、**解決済み allowlist（カテゴリ）+ member-lookup の per-caller ゲート設定（`memberLookupGate`）**、**当該応答の `sessionId`**〔conversation-context が解決済み〕、**fresh 解決した invoking member**〔ctx 構築時＝per-response に `guild.members.fetch({user, force:true, cache:false})` で 1 回解決し、**sync な per-caller ゲート（`isEnabled`）と read/management の権限チェックの双方が再利用**＝handler ごとの再 fetch を避けつつ stale な `message.member` を authz に使わない。response 内の数秒で role 変化は問題にしない〕、**loop スコープ state（`fetchMoreContextState` / `memberLookupState`）**）を載せる経路を追加（state は per-runToolLoop に構築。global/static にしない）
...
4. （`fetch_more_context` のみ）conversation-context が当該 guild で有効（ctx に scoped history reader が載っているか＝`ctx.historyReader != null` で同期判定。下記「接合」）
5. （`create_thread` のみ）`ctx.channel.type === ChannelType.GuildText`（news/forum/media とスレッド内は v1 非対応。理由は Decisions 参照）
6. （`search_members`/`member_info` のみ）member-lookup の **per-caller ゲート**（`ctx.memberLookupGate`＝guild 設定から ctx 構築時に解決済み）が未設定、または invoking member（ctx に **fresh 解決**した role/permission/`ctx.channelId`）がそれを満たす（許可ロール / `ManageGuild` / 許可チャンネル）。既定（ゲート未設定）は全 user 可。**このゲートは `isEnabled` に入れて基盤の dispatch 再評価（セキュリティ境界）に乗せる**（config だけ持って enforcement 経路に無いと opt-in 後に全 user が呼べてしまう）
...
`buildTools(ctx)` はこれを満たす tool だけを `tools` 配列に出す。`isEnabled(ctx)` は**同期**で、上記 1-6 の入力（allowlist / context 有効 or reader 有無 / per-caller ゲート設定 / channel / fresh invoking member / appFlags）は**すべて ctx 構築時に事前解決済み**の値だけを読む（isEnabled 内で DB/REST fetch をしない）。**基盤は dispatch 時に `isEnabled(ctx)` を再評価**するが、これは**同一の per-loop ctx に対する再評価**で、(a) 凍結スナップショットに無い tool 名（モデルの hallucination 等）と (b) ctx 由来ゲートが false の tool を実行させない defense-in-depth であって、**応答生成の途中で guild 設定が変わってもそれは反映しない**（allowlist/ゲート含む ctx は応答生成＝`runToolLoop` 1 実行ごとに再構築されるため、設定変更は次応答から効く）。さらに handler が実 Discord 権限を確認する（下記）。
...
| `search_members` | member-lookup | `guild.members.search({query, limit: min(args.limit, 25), cache:false})`（REST `Routes.guildMembersSearch`。**Discord endpoint cap は 1000 だが v1 tool schema は max 25・既定 10 で公開**） | intent/権限不要。consent は member-lookup opt-in（flag に紐付けない）+ 任意 per-caller ゲート | **off** |
...
- **content 不在の扱い（REST gating は要実装時検証）**: MessageContent portal トグル無効時に `fetch_recent_messages` が本文空で返りうる。**REST fetch が実際に gating されるかは Discord docs 上 contested**（canonical Message Resource は空 content ルールを GATEWAY イベント文脈に限定、field report〔#4552/#4775〕は REST も gating と報告）→ **実装時に実 API で確認**。tool を提示し続け content 欠落を明示する案（本設計）と、MessageContent 無効なら提示しない案のトレードオフ（本設計は前者。flag で hard-gate せず実 content 不在のときだけ注記するため gating 有無いずれでも頑健。bot 宛て以外の本文が見えないのは privacy 的にむしろ望ましい場面もある）。
