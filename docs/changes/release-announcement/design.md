...
- テスト: 実際の CHANGELOG.md での節の切り出し（先頭、末尾の注釈とリンク定義、`[Unreleased]`、壊れた見出し、重複、CRLF、コードブロックの中の見出し）、版の比較（prerelease の拒否）、範囲の途中の版の節が壊れているときに何も送らず記録も進めないこと、空の節、閉じていないコードブロック、読めない記録、新しい DB で動いている版の節が無い・重複している・prerelease のときに記録しないこと、初回・同じ版・ロールバックとその後の再アップグレード・複数版飛ばし、同時に起動した 2 つの claim のうち 1 つだけが送ること、動いている版の節が無いときに記録が進まないこと、1 つのサーバーで失敗しても他へ送ること、メンションの抑止、日本語の複数ページ、権限の無いチャンネル、`/release-note` の既定値と存在しない版

### 実装内容

- CHANGELOG は起動時に 1 回読み、プロセスの中で持つ。ファイルが無い、または読めなければ、起動時の通知を行わずにログに残す。`/release-note` はコマンドとして残し、読めない旨の notice を返し、autocomplete は候補を返さない。
