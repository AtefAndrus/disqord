...
- 本文中のカスタム絵文字を、モデルが読める `:name:` の形で渡す
- スタンプを、名前と説明の表記で渡し、画像として見られる形式のスタンプは画像としても渡す
- GIF の埋め込みを、表記と静止画のサムネイルで渡す
- 過去のメッセージのスタンプと GIF を `view_attachment` で開けるようにする
...
| 判断事項 | 選択 | 理由 |
| -------- | ---- | ---- |
| 本文中のカスタム絵文字 | `<:name:id>` と `<a:name:id>` を `:name:` に置き換えて渡す。画像は渡さない | 名前だけで意味が伝わることが多く、1 個ずつ画像にすると token が大きく増える。ID はモデルに不要である |
...
| 過去のスタンプと GIF | 窓に入った過去のメッセージでは表記だけを渡し、`view_attachment` の対象に含めて番号で開けるようにする | 過去の添付と同じく、必要なときだけモデルが取りに行く。添付の番号の後ろに続けて番号を振る |
| 画像の取得先 | スタンプは Discord CDN の `stickers/<id>.png`（GIF 形式は `media.discordapp.net/stickers/<id>.gif`）、GIF はサムネイルの URL だけを取得する。それ以外の URL は取得しない | 取得先を Discord の CDN と、Discord が作った埋め込みのサムネイルに限る。サムネイルは Discord のプロキシ（`proxy_url`）を優先する |
...
- 修正: `src/services/attachmentParser.ts` / `src/bot/events/messageCreate.ts` — bot を呼んだメッセージのスタンプと GIF のサムネイルを画像としてモデルへ渡す
...
- スタンプと GIF のサムネイルは、既存の画像と同じく、モデルが画像入力に対応しない（`isMultimodalCapable` が false）なら表記だけにする。
- `view_attachment` の番号は、添付を 1 から振った後に、スタンプ、GIF の順で続けて振る。表記に番号を含める（例 `[スタンプ m7/2: "name"]`）。

## Tasks

- [ ] `GuildExpressions` intent を足す
- [ ] カスタム絵文字、スタンプ、GIF の正規化と表記を実装する
- [ ] bot を呼んだメッセージのスタンプと GIF のサムネイルを画像として渡す
- [ ] 過去のスタンプと GIF を `view_attachment` で開けるようにする
