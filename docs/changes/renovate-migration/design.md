...
| 範囲指定の依存の更新 | `rangeStrategy: "bump"`。`lockFileMaintenance` は無効のまま | 既定の `rangeStrategy` は範囲を満たす新版を更新対象にしないため、`discord.js`（`^14.26.5`）と `zod`（`^4.4.3`）の lock が動かなくなる。`bump` なら範囲と lock が一緒に上がり、依存ごとの版が PR に出る。範囲を上げる更新の `updateType` は `minor` / `patch` のままで `isBump` が付くだけなので、catch-all のクールダウンも効く（Renovate 44.103.2 の local lookup で、クールダウンを 30 日にすると公開 8 日の `zod` 4.6.5 が `pendingChecks: true` になることを確認）。`config:best-practices` 由来の `security:minimumReleaseAgeNpm` は bump のクールダウンを外すが、後に並ぶ本リポジトリの packageRules が掛け直す。`lockFileMaintenance` は推移的な依存までまとめて動かし、クールダウンも掛からないので使わない |
...
- [x] customManagers の置換は、Renovate 44.54.0 の local synthetic dry-run で `1.7.11` と対応 digest から `1.7.12` と対応 digest への同時更新候補が出ることで確認した。本番の生成 PR での確認は actionlint の次のリリースまで行えないため、完了条件には含めない（Open Questions 参照）
...
- actionlint の customManagers による本番の生成 PR は、まだ一度も作られていない。actionlint は v1.7.12（2026-03-30）以降リリースが無く、置換の確認は local dry-run までにとどまる。regex は `ci.yml` の書式変更（クォート形式や空白の変更）で 0 件マッチに退化しうるが、その場合に失うのは actionlint の更新 PR だけなので、次のリリースで PR が来るかを見て判断する。
