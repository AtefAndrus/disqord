...
- 新規: `bunfig.toml` — `[install] minimumReleaseAge` を Renovate と同じ 3 日に設定（Phase 1）
...
```json5
{
  ...
  labels: ["dependencies"],
  packageRules: [
    // 供給網対策のクールダウン。version 更新に限る（pin / digest は releaseTimestamp を
    // 持たず永久 pending になるため対象外。公式の security:minimumReleaseAge* も同じ理由で
    // lockFileMaintenance / replacement / pin を除外している）。
    // 値は bunfig.toml の [install] minimumReleaseAge と揃えること。bun 側を長くすると
    // Renovate が提案した版を bun install が拒否して artifact 更新が壊れる
    {
