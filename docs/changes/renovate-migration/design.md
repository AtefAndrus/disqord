...
```json5
{
  ...
  rangeStrategy: "pin", // 実行時の依存も exact に固定する（Decisions「実行時の依存の版指定」参照）
  ...
  packageRules: [
    ...
    // 「型は通るが実行時に存在しない API」を受け入れてしまう。CI の drift-check は bun.lock の bun-types も
    // 含めて patch の一致を要求するので、別 PR に割れると片側の PR が必ず落ちる。同じ PR に載せる。
    //
    {
    ...
    // :pinDevDependencies による range -> exact の一度きりの変換。bun マネージャは lockedVersion を
    // 持たないので範囲内の最新へ固定し、版が動きうる (pin はクールダウンの対象外)。
...
- 検証は 3 段階で行う: ① 構文は `renovate-config-validator`、② 抽出結果（regex が 0 件マッチに退化していないか）と更新候補は Renovate の local dry-run、③ 本番での置換結果は生成 PR の差分と CI 成功。①と②は確認済みで、③は actionlint の次のリリースで PR が来たときに見る（Open Questions 参照）。
...
- [x] 週次グループ PR を 2 サイクル確認した。schedule（月曜 0〜8 時台 JST）どおり、2026-09-07 03:13 JST に #96（`all-minor-patch`）と #97（`zizmor`）、2026-09-14 01:55 JST に #106（`bun-toolchain`）と #107（`all-minor-patch`）が作られ、グルーピングは設計どおりだった。releaseTimestamp の取得可否は、Renovate 44.103.2 の local lookup（`--platform=local --dry-run=lookup`、2026-09-20）で datasource ごとに確認した: `mise.toml` の bun（mise マネージャ、datasource は `github-releases`）、`oven/bun`（docker）、mise-action の `jdx/mise`（`github-release-attachments`）、`zizmorcore/zizmor-action`（`github-tags`）、`rhysd/actionlint`（docker）は取得でき、`ghcr.io/zizmorcore/zizmor`（docker / GHCR）だけが取得できない。GHCR の CLI だけをクールダウンの例外にしている現行設定と一致する。minimumReleaseAge は、同じ lookup で公開から 3 日未満の `jdx/mise` `v2026.9.11` が `pendingChecks: true` になり、更新候補から保留されることで確認した
...
- [x] customManagers の抽出と更新候補を、Renovate 44.54.0 の local synthetic dry-run で確認した（`1.7.11` と対応 digest から `1.7.12` と対応 digest への同時更新候補）。実ファイルの置換と CI の検証は完了条件から外し、未検証のリスクとして受容する（Open Questions 参照）
...
- **Dashboard に `Could not re-extract the packageFile after updating it` が出るが、これは上流の構造的な制約で対処不要**。この警告は `checkForPendingVersions`（`lib/workers/repository/update/branch/get-updated.ts`）から出る。`minimumReleaseAge` で保留中の新しい版がある依存について、lockfile 更新の結果が Renovate の想定と食い違っていないかを検証する安全確認で、汎用の `extractPackageFile` を呼ぶ。しかし `bun` マネージャは `extractAllPackageFiles` だけを export していて `extractPackageFile` を持たない（`lib/modules/manager/bun/index.ts`。`npm` マネージャも同じ）ため、再抽出が必ず null を返し警告になる。設定側で回避する手段はない（上流 [renovate#41624](https://github.com/renovatebot/renovate/issues/41624) が対応対象）。
  - 実害は「lockfile が想定と違う版を引いていないかの検証が効かない」ことに限られる。本リポジトリの直接依存は devDependencies（`:pinDevDependencies`）も実行時の依存（`rangeStrategy: "pin"`）も exact に固定されるため、直接依存について `bun install` に選択の余地はない。残るのは推移的な依存の範囲解決である。
...
- actionlint の customManagers による生成 PR は、まだ一度も作られていない。確認できているのは local dry-run での抽出と更新候補までで、実ファイルの置換と CI は未検証である。Design「Phase 2」の合成手順を使えば確かめられるが、actionlint は v1.7.12（2026-03-30）以降リリースが無く、手順の手間に見合わないとして行っていない。起こりうる失敗は 2 つある。regex が `ci.yml` の書式変更で 0 件マッチに退化すれば、actionlint の更新 PR が来なくなる。置換が壊れれば、actionlint を含む週次の non-major グループ PR の CI が落ちる。後者は PR の CI で目に見えるので、その時点で直す。
...
- CI の drift-check は `bun.lock` が解決した `bun-types` の版も比較し、`mise.toml` / `Dockerfile` との patch の完全一致を要求する（`.github/workflows/ci.yml` の bun drift check）。グループ化は同じ PR に載せることしか保証しないので、版の一致はこの検査で担保する。代わりに、bun のある patch に対応する `bun-types` が公開されなかった場合は、その patch への更新が drift-check で止まり、手で対処することになる。
