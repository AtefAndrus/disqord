...
- [x] customManagers の抽出と更新候補を、Renovate 44.54.0 の local synthetic dry-run で確認した（`1.7.11` と対応 digest から `1.7.12` と対応 digest への同時更新候補）。実ファイルの置換と CI は本番の生成 PR でしか確かめられず、actionlint は v1.7.12（2026-03-30）以降リリースが無い。合成手順は影響が更新漏れに限られるため使わず、本番の確認は完了条件に含めない（Open Questions 参照）
...
- actionlint の customManagers による本番の生成 PR は、まだ一度も作られていない。確認できているのは local dry-run での抽出と更新候補までで、実ファイルの置換と CI は未検証である。regex は `ci.yml` の書式変更（クォート形式や空白の変更）で 0 件マッチに退化しうるが、その場合に失うのは actionlint の更新 PR だけである。actionlint の次のリリースで Renovate の PR が来たら、差分でタグと digest が揃って置換されていることを確かめる。
