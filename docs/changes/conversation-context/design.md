...
| 明示的なキャッシュ | `cache_control` などは送らない | Gemini の明示的なキャッシュは書き込みと 5 分間の保存に料金が掛かる。暗黙のキャッシュは `google/gemini-3.8-flash` では観測できなかった（下記 7）ので、費用の削減はどちらも当てにせず、まず会話あたりの費用を見る |
...
- 2026-09-23 に `google/gemini-3.8-flash` で実測したところ、キャッシュの読み取りは観測できなかった。同じ窓で続けて 10 回尋ね、入力 1,178〜6,784 トークンのすべてのターンで `cached_tokens` は 0 だった。
- 原因は窓の作りではない。OpenRouter へ直接、同一の前置き（3,405 トークンと 11,905 トークン）で連続してリクエストしても `cached_tokens` は 0 で、プロバイダを Google AI Studio と Vertex に固定しても同じだった。`prompt_tokens_details` は返っているので、値が 0 であることは報告の欠落ではない。
- したがって、このモデルでは暗黙のキャッシュによる費用の削減は当てにできない。窓の開始位置を固定する理由は、キャッシュの命中ではなく、同じ会話の間に送る内容を安定させ、再取得と入力の揺れを抑えることである。
- 明示的なキャッシュ（`cache_control`）を使うかは未決とする。書き込みと保存に料金が掛かるので、会話あたりの費用と窓の再利用率を見てから決める。
