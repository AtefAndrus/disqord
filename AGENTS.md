...
- The `/config` settings the scenarios need can be switched without a Discord client: they are columns of `guild_settings` in the bot's `DATABASE_PATH` (`show_llm_details`, `web_search_enabled`, `history_enabled`, `reasoning_display_enabled`), and `SettingsService` reads them from the database on every request, so `sqlite3 data/disqord.db "update guild_settings set web_search_enabled=0 where guild_id='<guild of E2E_CHANNEL_ID>'"` before the run takes effect. Note the old values and restore them afterwards. This is how a code path that depends on a setting, such as the no-search path, is exercised end to end.
