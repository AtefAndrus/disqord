...
- `bun run e2e search` runs only when named: it needs `/config web-search on` in the guild under test (turn it off afterwards) and pays for each search. It checks the date of a fixed past release, so a model that already knows the date still passes; `Searches: N` in the footer is what shows the search ran. `google/gemini-3.5-flash-lite` sometimes gets the year wrong even after searching.
