...
- `bun run e2e reasoning` runs only when named and needs `/config reasoning-display on` and `/config history on` in the guild under test (turn them off afterwards) and a model/provider that returns a reasoning summary or text; the default free model normally does not. It makes the model call `read_earlier_messages` so that the next turn sends the reasoning items back, then checks that the final page carries `reasoning.md`. Run it before merging a PR that changes how reasoning is requested, received, resent, or attached.
