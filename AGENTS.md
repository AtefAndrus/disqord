...
- `bun run e2e history-set history-recall` runs only when named, and both must be named in one run: history-set tells the bot a passphrase that is random per run and history-recall asks for it back, so only stored conversation history can make it pass. It needs `/config history on` in the guild under test (turn it off afterwards, which also purges what the run stored). Run it, in addition to the default scenarios, before merging a PR that changes how history is stored, selected, or sent (`src/db/repositories/conversation.ts`, `src/services/historyRecorder.ts`, or the history paths in `chatService.ts` and `messageCreate.ts`).
