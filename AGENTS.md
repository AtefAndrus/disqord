...
- `bun run e2e reasoning` runs only when named and needs `/config reasoning-display on` in the guild under test and a model/provider that supports reasoning and actually returns a summary or reasoning text; the default free model normally does not return displayable reasoning.
