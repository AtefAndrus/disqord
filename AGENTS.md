...
- `bun run e2e reasoning` runs only when named and needs `/config reasoning-display on` and `/config history on` in the guild under test (turn them off afterwards). It makes the model call `read_earlier_messages` so that the next turn sends the reasoning items back, then checks that the first page carries the reasoning TextDisplay (found by its component id) above the answer. Run it with a model that returns reasoning text on every reply: `z-ai/glm-5.3-flash` does, while `google/gemini-3.8-flash` and `openai/gpt-6-luna` often return only encrypted reasoning and then fail the check without a code fault (2026-09-24). Run it before merging a PR that changes how reasoning is requested, received, resent, or shown.
